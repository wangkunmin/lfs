...
export class LevelTypeComponent implements OnInit {
  ...
  getWatch($event?, companyName?) {
    ...
    this.api.findAllRiskRateInfo( this.currentPage,this.itemsPerPage,companyName)
      .subscribe(
  ...
  //公司集合信息空数据处理
  setWatchInfo(data: RiskRateIn[]) {
    for (let i = 0; i < data.length; i++) {
       let rating = data[i].rating;
       if([1,2,3,4].indexOf(rating)<0){
         data[i].ratingSrc = '';
       }
      for(let ratingTemp of Base.comRiskLevels){
         if(rating == ratingTemp.key){
           data[i].ratingSrc = ratingTemp.value;
         }
      }
    }
    this.watchInfo = data;
