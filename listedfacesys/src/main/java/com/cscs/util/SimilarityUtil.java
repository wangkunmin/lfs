...
public class SimilarityUtil {
    ...
    public static boolean isSimilar(CharSequence s, CharSequence t){
        return getSimilarity(s, t) > 0.5999;
    }

    public static float getSimilarity(CharSequence s, CharSequence t){
        if(s == null || t == null){
            return 0;
        }

        float distance = getLevenshteinDistance(s, t);
        float length = Math.max(s.length(), t.length());
