package com.cscs.redisCommon.interceptor;
...
import com.cscs.redisCommon.util.RedisUtil;
...
/**
 * 切面工具类
 * 给不同的方法来加入判断如果缓存存在数据，从缓存中取数据，否则从数据库中取，并将结果保存达到缓存中去
 * 参考资料 https://m.2cto.com/kf/201512/455170.html
 */
public class MethodCacheInterceptor implements MethodInterceptor {
    ...
    public MethodCacheInterceptor() {
        try {
//            File f = new File("D:\\lunaJee-workspace\\msm\\msm_core\\src\\main\\java\\com\\mucfc\\msm\\common\\cacheConf.properties");  //配置文件位置直接被写死，有需要自己修改下
//            InputStream in = new FileInputStream(f);
            InputStream in = getClass().getClassLoader().
                    getResourceAsStream(
                            "cacheConf.properties");
                    ...
                    public void run() {
                        if (tkey.startsWith("com.service.impl.xxxRecordManager")) {
                            redisUtil.set(tkey, tvalue, xxxRecordManagerTime);
                                ...
                                tkey.startsWith("com.service.impl.xxxSetRecordManager")) {
    ...
    /** * 创建缓存key * * @param targetName * @param methodName * @param arguments */
